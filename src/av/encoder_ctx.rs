...
impl EncoderCtx {
    ...
    pub fn load_stream(&mut self, decoder_ctx: &DecoderCtx, codec_id: u32) {
        unsafe {
    ...
    pub fn open_file(&mut self, path: &str) {
        ...
        let response = unsafe { sys::avformat_write_header(self.av, null_mut()) };

        utils::check_error(response);
    }

    pub fn encode(&mut self, decoder_ctx: &DecoderCtx, frame: &*mut sys::AVFrame) -> i32 {
        unsafe {
